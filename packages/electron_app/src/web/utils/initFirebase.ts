...
if (!projectId) {
  throw new Error("can not initialze firebase app, because projectId is undefined");
}
if (!storageBucket) {
  throw new Error("can not initialze firebase app, because storageBucket is undefined");
}
if (!msgSenderId) {
  throw new Error("can not initialze firebase app, because msgSenderId is undefined");
}
if (!appId) {
...
export const initFirebase = (mode: string) => {
  return new Promise<{ firebaseApp?: FirebaseApp; auth: Auth; fdb: Firestore }>(
    (resolve, reject) => {
      try {
        if (mode === "production") {
          ...
          resolve({ firebaseApp: firebaseApp, auth: auth, fdb: fdb });
